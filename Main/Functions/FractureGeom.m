...
%     end
    ...
    % 1D pattern
    PatternIn = max(min(PatternIn,nHigh),nLow);
    
    x_list = XGrid(1);
    index_list = PatternIn(1);
    
    for ii=2:Nx
        dn = PatternIn(ii)-index_list(end);
        if abs(dn)>Tolerance
            x_list = [x_list, XGrid(ii)];
            index_list = [index_list, PatternIn(ii)];
        else
            x_list(end) = XGrid(ii);
        end
    end
    
    GeometryOut = {x_list,index_list};
end
end